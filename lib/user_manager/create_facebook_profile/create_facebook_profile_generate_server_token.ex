...
"""
  def handle_events(events, from, state) do
    ...
    |> Flow.map(fn e -> parse_server_token(e) end)
    |> Enum.to_list
  ...
  defp process_event({:process_access_token, token, expire_time, user_id, notify}) do
    ...
    response_json = Poison.Parser.parse!(response)
      case status_code == 200 do
        ...
        false -> {:server_token_error, token, expire_time, response_json, status_code, user_id, notify}
      end
  end
  defp parse_server_token({:parse_server_token, token, expire_time, response_json, user_id, notify}) do
    case Map.get(response_json, "access_token", "") do
      "" -> {:server_token_missing, token, expire_time, response_json, user_id, notify}
      server_token ->
        case Map.get(response_json, "expires_in", -1) do
          -1 -> {:expire_time_missing, token, expire_time, response_json, user_id, notify}
          other -> {:process_server_token, token, expire_time , server_token, other, user_id, notify}
        end
    end
  end
  defp parse_server_token({:server_token_error, token, expire_time, response_json, status_code, user_id, notify}) do
