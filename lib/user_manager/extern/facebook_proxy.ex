defmodule UserManager.Extern.FacebookProxy do
  ...
  def handle_call({:get_me, field_names_string, access_key}, _from, state) do
    {:json, res} = Facebook.me(field_names_string, access_key)
    {:reply, {:ok, res}, []}
  end
  def handle_call({:get_server_token_from_access_key, token}, _from, state) do
  ...
  def handle_call({:get_access_key_from_code, code_token}, _from, state) do
