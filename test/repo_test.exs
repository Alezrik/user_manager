defmodule RepoTest do
  use ExUnit.Case
    ...
    test "create user with a permission from valid changeset" do
      ...
      user_profile = UserProfile.changeset(%UserProfile{}, %{"authentication_metadata" => %{"credentials" => %{"name" => Faker.Name.first_name <> Faker.Name.last_name, "password" => <<"somelegalpassword">>, "email" => Faker.Internet.email}}})
      user = UserSchema.changeset(%UserSchema{}, %{}) |> put_assoc(:permissions, [permission_serialized]) |> put_assoc(:user_profile, user_profile)
      assert user.valid?, user.errors
      {:ok, insert} = Repo.insert(user)
    end

    test "create user profile with a permission from valid changesets" do
      ...
      assert permission.valid?
      user_profile = UserProfile.changeset(%UserProfile{}, %{"authentication_metadata" => %{"credentials" => %{"name" => Faker.Name.first_name <> Faker.Name.last_name, "password" => <<"somelegalpassword">>, "email" => Faker.Internet.email}}})
      user = UserSchema.changeset(%UserSchema{}, %{}) |> put_assoc(:permissions, [permission]) |> put_assoc(:user_profile, user_profile)
      assert user.valid?, user.errors
      {:ok, insert} = Repo.insert(user)
    end
end
